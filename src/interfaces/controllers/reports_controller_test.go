...
func TestReportsController_UpdateWeeklyPerTag_ReturnNilWhenSucceeded(t *testing.T) {
	...
	mir.On("GetAll", time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC), "TAG").Return(&domain.Items{{Title: "TITLE"}}, nil)
	...
	mrr.On("Update", "REPORT_ID", "BODY", "TAG").Return(nil)
...
func TestReportsController_UpdateWeeklyPerTag_ReturnErrorWhenGetAllFailed(t *testing.T) {
	mir := new(mockItemsRepository)
	mir.On("GetAll", time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC), "TAG").Return((*domain.Items)(nil), errors.New("SOMETHING_WRONG"))
...
func TestReportsController_UpdateWeeklyPerTag_ReturnUpdateFailed(t *testing.T) {
	...
	mrr := new(mockReportsRepository)
	mrr.On("Update", "REPORT_ID", "BODY", "TAG").Return(errors.New("SOMETHING_WRONG"))
