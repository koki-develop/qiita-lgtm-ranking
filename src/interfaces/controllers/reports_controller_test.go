...
func TestReportsController_UpdateWeekly_ReturnNilWhenSucceeded(t *testing.T) {
	...
	mir.On("GetAll", time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC)).Return(&domain.Items{{Title: "TITLE"}}, nil)
...
func TestReportsController_UpdateWeekly_ReturnErrorWhenGetAllFailed(t *testing.T) {
	mir := new(mockItemsRepository)
	mir.On("GetAll", time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC)).Return((*domain.Items)(nil), errors.New("SOMETHING_WRONG"))
