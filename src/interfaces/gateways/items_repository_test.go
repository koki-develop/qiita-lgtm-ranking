...
func TestItemsRepository_GetAll_ReturnItemsWhenSucceeded(t *testing.T) {
	mapi := new(mockQiitaAPI)
	mapi.On("GetItems", 1, 100, "QUERY").Return(&domain.Items{{Title: "TITLE_1"}}, nil)
	mapi.On("GetItems", 2, 100, "QUERY").Return(&domain.Items{{Title: "TITLE_2"}}, nil)
	mapi.On("GetItems", 3, 100, "QUERY").Return(&domain.Items{}, nil)
...
func TestItemsRepository_GetAll_ReturnErrorWhenGetItemsFailed(t *testing.T) {
	mapi := new(mockQiitaAPI)
	mapi.On("GetItems", 1, 100, "QUERY").Return((*domain.Items)(nil), errors.New("SOMETHING_WRONG"))
